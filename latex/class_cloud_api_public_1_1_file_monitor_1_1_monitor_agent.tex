...
\hline
{\em on\-Queueing\-Callback} & (optional) action to be executed every time a File\-Change would be queued for processing\\
\hline
{\em log\-Processing} & (optional) if set, logs File\-Change objects when their processing callback fires\\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
Returns any error that occurred, or null.
\end{DoxyReturn}
...
A push notification has been received. Starts the queue timer to start sync processing, if it is not already started for other events 
