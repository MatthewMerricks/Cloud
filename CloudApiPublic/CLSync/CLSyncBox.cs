...
namespace Cloud
{
    ...
    public sealed class CLSyncbox
    {
        ...
        /// <summary>
        /// Migrates an existing local Syncbox from an old path, if any already existed
        /// </summary>
        /// <param name="oldPath">Full path where the Syncbox previously existed</param>
        /// <param name="syncboxId">Syncbox id for this syncbox</param>
        /// <param name="settingsAtNewPath">Settings including required SyncRoot and DeviceId for the new path of the syncbox</param>
        /// <param name="status">(output) Status of migrating the syncbox path</param>
        /// <returns>Returns any error which occurred migrating the syncbox path, if any</returns>
        public static CLError MoveLocalSyncboxFromOldPathToNewPath(
            string oldPath,
            long syncboxId,
            ICLSyncSettings settingsAtNewPath,
            out CLSyncboxMovePathStatus status)
        {
            status = CLSyncboxMovePathStatus.ErrorUnknown;
            try
            {
                if (Helpers.AllHaltedOnUnrecoverableError)
                {
                ...
                if (settingsAtNewPath == null)
                {
                    status = CLSyncboxMovePathStatus.ErrorNullSettingsAtNewPath;
                    throw new ArgumentException("settingsAtNewPath cannot be null");
                }

                if (string.IsNullOrEmpty(oldPath))
                {
                    status = CLSyncboxMovePathStatus.ErrorSyncboxFolderDoesNotExistAtOldPath;
                    throw new ArgumentException("oldPath must be specified");
                }

                ICLSyncSettingsAdvanced copiedSettings = settingsAtNewPath.CopySettings();

                if (string.IsNullOrEmpty(copiedSettings.DeviceId))
                {
                    const string settingsError = "settingsAtNewPath DeviceId cannot be null";
                    status = CLSyncboxMovePathStatus.ErrorNullDeviceId;
                    throw new NullReferenceException(settingsError);
                }

                // Check the TraceLocation vs. LogErrors
                if (string.IsNullOrWhiteSpace(copiedSettings.TraceLocation) && copiedSettings.LogErrors)
                {
                    const string verifyTraceError = "TraceLocation must be set if LogErrors is checked";
                    status = CLSyncboxMovePathStatus.ErrorTraceEnabledWithoutDirectory;
                    throw new ArgumentException(verifyTraceError);
                }

                //// DO NOT MOVE THIS EARLIER EVEN THOUGH EARLIER STATEMENTS HAVE TRACE
                // Initialize trace in case it is not already initialized.
                CLTrace.Initialize(copiedSettings.TraceLocation, "Cloud", "log", copiedSettings.TraceLevel, copiedSettings.LogErrors);

                if (!String.IsNullOrWhiteSpace(copiedSettings.DatabaseFolder))
                {
                    FilePath fpDatabase = copiedSettings.DatabaseFolder;
                    FilePath fpSyncbox = copiedSettings.SyncRoot;
                    if (fpDatabase.Contains(fpSyncbox, insensitiveNameSearch: true))
                    {
                        const string verifyDatabaseError = "settingsAtNewPath DatabaseFolder cannot be inside the settingsAtNewPath SyncRoot";
                        status = CLSyncboxMovePathStatus.ErrorDatabaseFolderInsideSyncboxFolder;
                        throw new ArgumentException(verifyDatabaseError);
                    }
                }

                if (!string.IsNullOrWhiteSpace(copiedSettings.TraceLocation))
                {
                    FilePath fpTraceLocation = copiedSettings.TraceLocation;
                    FilePath fpSyncbox = copiedSettings.SyncRoot;
                    if (fpTraceLocation.Contains(fpSyncbox, insensitiveNameSearch: true))
                    {
                        const string verifyTraceLocationError = "settingsAtNewPath TraceLocation cannot be inside the Syncbox settings SyncRoot";
                        status = CLSyncboxMovePathStatus.ErrorTraceFolderInsideSyncboxFolder;
                        throw new ArgumentException(verifyTraceLocationError);
                    }
                }

                CLError checkBadPath = Helpers.CheckForBadPath(copiedSettings.SyncRoot);
                if (checkBadPath != null)
                {
                    status = CLSyncboxMovePathStatus.ErrorBadRootPath;
                    throw new ArgumentException("SyncRoot in settingsAtNewPath represents a bad path, check it first via Helpers.CheckForBadPath", checkBadPath.GrabFirstException());
                }

                int tooLongChars;
                CLError checkPathLength = Helpers.CheckSyncRootLength(copiedSettings.SyncRoot, out tooLongChars);
                if (checkPathLength != null)
                {
                    status = CLSyncboxMovePathStatus.ErrorLongRootPath;
                    throw new ArgumentException("SyncRoot in settings is too long, check it first via Helpers.CheckSyncRootLength", checkPathLength.GrabFirstException());
                }
                ...
                FileInfo dbInfo;
                string oldCalculatedFullPath;

                string indexDBLocation = Helpers.CalculateDatabasePath(copiedSettings.DatabaseFolder, copiedSettings.DeviceId, syncboxId);
                IndexingAgent.CheckDatabaseFileState(createEvenIfExisting: false, dbInfo: out dbInfo, dbNeedsDeletion: out dbNeedsDeletion, dbNeedsCreation: out dbNeedsCreation, indexDBLocation: indexDBLocation, rootObjectCalculatedFullPath: out oldCalculatedFullPath);

                if (dbNeedsCreation)
                {
                    status = CLSyncboxMovePathStatus.Success;
                }
                else
                {
                    if (oldPath != oldCalculatedFullPath)
                    {
                        status = CLSyncboxMovePathStatus.ErrorSyncboxFolderDoesNotExistAtOldPath;
                        throw new ArgumentException("oldPath does not match the last known SyncRoot");
                    }

                    IndexingAgent indexer;

                    // Start the indexer.
                    const string createIndexerErrorString = "Unable to create indexer to move SyncRoot";
                    CLCredential fakeCred;
                    CLCredentialCreationStatus fakeCredStatus;
                    CLError fakeCredCreationError = CLCredential.CreateAndInitialize(
                        "0",
                        "0",
                        out fakeCred,
                        out fakeCredStatus);

                    if (fakeCredStatus != CLCredentialCreationStatus.Success)
                    {
                        status = CLSyncboxMovePathStatus.ErrorIndexCreation;
                        throw new Exception(createIndexerErrorString);
                    }

                    CLSyncboxCreationStatus notUsedStatus = CLSyncboxCreationStatus.Success;
                    CLError indexCreationError = IndexingAgent.CreateNewAndInitialize(
                            out indexer,
                            new CLSyncbox(
                                fakeCred,
                                syncboxId,
                    ...
                    CLError changeRootError = indexer.ChangeSyncRoot(copiedSettings.SyncRoot);
                    if (changeRootError != null)
                    {
                        throw new Exception("Error performing change to SyncRoot");
                    }
    ...
    public enum CLSyncboxMovePathStatus : byte
    {
        Success = 0,
        ErrorUnknown = 1,
        ErrorNotFoundAtOldPath = 2,
        ErrorTraceEnabledWithoutDirectory = 3,
        ErrorDatabaseFolderInsideSyncboxFolder = 4,
        ErrorTraceFolderInsideSyncboxFolder = 5,
        ErrorNullDeviceId = 6,
        ErrorLongRootPath = 7,
        ErrorBadRootPath = 8,
        ErrorIndexCreation = 9,
        ErrorSyncboxFolderDoesNotExistAtOldPath = 10,
        ErrorSyncboxFolderDoesNotExistAtNewPath = 11,
        ErrorNullSettingsAtNewPath = 12
    }
}