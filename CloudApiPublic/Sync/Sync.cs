...
namespace CloudApiPublic.Sync
{
    ...
    public sealed class SyncEngine : IDisposable
    {
        ...
        private static readonly string DefaultTempDownloadsPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData, Environment.SpecialFolderOption.Create) + "\\" +
            Helpers.GetDefaultName() + // name of currently running application
            "\\DownloadTemp";
        ...
        // helper method which takes a FileChange and a hash string and creates a new FileChange with dependencies and sets the hash as the MD5 bytes
        private static FileChangeWithDependencies CreateFileChangeFromBaseChangePlusHash(FileChange baseChange, string hashString)
        {
            ...
            {
                byte[] hexBuffer = Helpers.ParseHexadecimalStringToByteArray(hashString);
