...
namespace CloudApiPublic.Sync
{
    public sealed class SyncEngine : IDisposable
    {
        ...
        private readonly ISyncSettings syncSettings;
                    ...
                    else
                    {
                        ...
                        foreach (PossiblyStreamableFileChangeWithUploadDownloadTask asyncTask in asyncTasksToRun)
                        {
                            try
                            {
                                ...
                                lock (UpDownEventLocker)
                                {
                                    UpDownEvent += asyncTask.FileChange.FileChange.FileChange_UpDown;
                                }
                                try
                                {
        ...
        private void RequeueFailures(List<PossiblyStreamableAndPossiblyPreexistingErrorFileChange> errorsToQueue,
                            ...
                            foreach (PossiblyStreamableAndPossiblyPreexistingErrorFileChange errorToQueue in errorsToQueue)
                            {
                                try
                                {
                                    if (successfulEventIds.BinarySearch(errorToQueue.FileChange.EventId) < 0)
                                    {
                                        ...
                                        if (ContinueToRetry(new PossiblyPreexistingFileChangeInError(errorToQueue.IsPreexisting, errorToQueue.FileChange), syncData, MaxNumberOfFailureRetries, MaxNumberOfNotFounds))
                                        {
        ...
        private Exception CompleteFileChange(PossiblyStreamableFileChange toComplete,
            ...
            try
            {
                            ...
                            || toComplete.FileChange.Type == FileChangeType.Modified))
                    {
                        ...
                        else
                        {
                            ...
                            asyncTask = new AsyncUploadDownloadTask(SyncDirection.From,
                                new Task<EventIdAndCompletionProcessor>(DownloadForTask,
                ...
                else
                {
                    ...
                    asyncTask = new AsyncUploadDownloadTask(SyncDirection.To,
                        new Task<EventIdAndCompletionProcessor>(UploadForTask,
                            new UploadTaskState()
                            {
                                FailureTimer = FailureTimer,
        ...
        private static EventIdAndCompletionProcessor UploadForTask(object uploadState)
        {
            ...
            try
            {
                try
                {
                    ...
                    uploadRequest.Timeout = (int)castState.HttpTimeoutMilliseconds;
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExecutableException<PossiblyStreamableFileChangeWithSyncData> wrappedEx = new ExecutableException<PossiblyStreamableFileChangeWithSyncData>(ProcessUploadError,
                        new PossiblyStreamableFileChangeWithSyncData(castState.FailedChangesQueue,
                            (byte)castState.MaxNumberOfFailureRetries,
                            (byte)castState.MaxNumberOfNotFounds,
                            castState.FailureTimer,
                            new PossiblyStreamableFileChange(castState.FileToUpload,
        ...
        private static void ProcessUploadError(PossiblyStreamableFileChangeWithSyncData exceptionState, AggregateException exceptions)
        {
            try
            {
                ...
                if (ContinueToRetry(new PossiblyPreexistingFileChangeInError(false, exceptionState.FileChange.FileChange), exceptionState.SyncData, exceptionState.MaxNumberOfFailureRetries, exceptionState.MaxNumberOfNotFounds))
                {
        ...
        private static EventIdAndCompletionProcessor DownloadForTask(object downloadState)
        {
            ...
            try
            {
                ...
                if (newTempFile != null)
                {
                    ...
                    castState.MoveCompletedDownload(newTempFileString,
                ...
                downloadRequest.Timeout = (int)castState.HttpTimeoutMilliseconds;
            ...
            catch (Exception ex)
            {
                ...
                {
                    ExecutableException<PossiblyStreamableFileChangeWithSyncData> wrappedEx = new ExecutableException<PossiblyStreamableFileChangeWithSyncData>(ProcessDownloadError,
                        new PossiblyStreamableFileChangeWithSyncData(castState.FailedChangesQueue,
                            (byte)castState.MaxNumberOfFailureRetries,
                            (byte)castState.MaxNumberOfNotFounds,
                            castState.FailureTimer,
                            new PossiblyStreamableFileChange(castState.FileToDownload, null),
                            castState.SyncData,
                            castState.SyncSettings,
                            castState.TempDownloadFolderPath,
                            newTempFile),
        ...
        private static void ProcessDownloadError(PossiblyStreamableFileChangeWithSyncData exceptionState, AggregateException exceptions)
        {
            try
            {
                ...
                if (ContinueToRetry(new PossiblyPreexistingFileChangeInError(false, exceptionState.FileChange.FileChange), exceptionState.SyncData, exceptionState.MaxNumberOfFailureRetries, exceptionState.MaxNumberOfNotFounds))
                {
                    ProcessingQueuesTimer downloadErrorTimer = exceptionState.DownloadErrorTimer;
                ...
                MessageEvents.FireNewEventMessage(exceptionState.FileChange,
                    growlErrorMessage,
                    EventMessageLevel.Regular,
                    true);
            }
        ...
        {
            public CancellationTokenSource FullShutdownToken { get; private set; }
            ...
            public AsyncRequestHolder(CancellationTokenSource FullShutdownToken)
            {
                ...
                this.FullShutdownToken = FullShutdownToken;
            }
                ...
                if (communicationArray.Length > 0)
                {
                            ...
                            Events = currentBatch.Select(currentEvent => new Event()
                            {
                                ...
                                Metadata = new Metadata()
                                {
                                    ...
                                    RelativeFromPath = (currentEvent.FileChange.OldPath == null
                                        ? null
                                        : currentEvent.FileChange.OldPath.GetRelativePath((syncSettings.CloudRoot ?? string.Empty), true)),
                                    RelativePath = currentEvent.FileChange.NewPath.GetRelativePath((syncSettings.CloudRoot ?? string.Empty), true),
                                    RelativeToPath = currentEvent.FileChange.NewPath.GetRelativePath((syncSettings.CloudRoot ?? string.Empty), true),
                                    ...
                                    TargetPath = (currentEvent.FileChange.Metadata.LinkTargetPath == null
                                        ? null
                                        : currentEvent.FileChange.Metadata.LinkTargetPath.GetRelativePath((syncSettings.CloudRoot ?? string.Empty), true))
                                }
                            }).ToArray()
                        };
                    ...
                    if (deserializedResponse.Events.Length > 0)
                    {
                        ...
                        foreach (int currentEventIndex in fromEvents)
                        {
                            try
                            {
                                if (eventsByPath.Contains((syncSettings.CloudRoot ?? string.Empty) + "\\" + (deserializedResponse.Events[currentEventIndex].Metadata.RelativePath ?? deserializedResponse.Events[currentEventIndex].Metadata.RelativeToPath).Replace('/', '\\')))
                                {
                        ...
                        if (duplicatedEvents.BinarySearch(currentEventIndex) < 0)
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    findHashableProperties = new FileMetadataHashableProperties(currentEvent.Metadata.IsFolder ?? ParseEventStringToIsFolder(currentEvent.Header.Action ?? currentEvent.Action),
                                    ...
                                    findLinkTargetPath = (string.IsNullOrEmpty(currentEvent.Metadata.TargetPath)
                                        ? null
                                        : (syncSettings.CloudRoot ?? string.Empty) + "\\" + currentEvent.Metadata.TargetPath.Replace("/", "\\"));
                                ...
                                Nullable<PossiblyStreamableFileChange> matchedChange = usePreviousFileChange
                                ...
                                {
                                    HashableProperties = findHashableProperties,
                                    LinkTargetPath = findLinkTargetPath,
                                        ...
                                        if (fileChangeForOriginalMetadata == null)
                                        {
                                            ...
                                            if (syncStateMetadata == null)
                                            {
                                                if (currentChange.Direction == SyncDirection.From)
                                                {
                                                    ...
                                                    FileChangeWithDependencies newPathCreation;
                                                    CLError newPathCreationDependenciesError = FileChangeWithDependencies.CreateAndInitialize(
                                                        CreateFileChangeFromBaseChangePlusHash(new FileChange()
                                                            {
                                                                ...
                                                                Metadata = new FileMetadata()
                                                                {
                                                                    ...
                                                                    HashableProperties = new FileMetadataHashableProperties(currentChange.Metadata.HashableProperties.IsFolder,
                                                                        newMetadata.ModifiedDate,
                                                                        newMetadata.CreatedDate,
                                                                        newMetadata.Size),
                                                                    Revision = newMetadata.Revision,
                                                                    StorageKey = newMetadata.StorageKey,
                                                                    LinkTargetPath = (newMetadata.TargetPath == null
                                                                        ? null
                                                                        : (syncSettings.CloudRoot ?? string.Empty) + "\\" + newMetadata.TargetPath.Replace("/", "\\"))
                                                                }
                                                            },
                                                            newMetadata.Hash),
                                                        null, out newPathCreation);
                                                    ...
                                                    {
                                                        PossiblyStreamableAndPossiblyChangedFileChangeWithError notFoundChange = new PossiblyStreamableAndPossiblyChangedFileChangeWithError(false,
                                                            newPathCreation,
                                                            null,
                                                            new Exception("Unable to find metadata for file. May have been a rename on a local file path that does not exist. Created new FileChange to download file"));
                ...
                else
                {
                    ...
                    if (respondingToPushNotification)
                    {
                        ...
                        incompleteChanges = deserializedResponse.Events.Select(currentEvent => new PossiblyStreamableAndPossiblyChangedFileChange(/* needs to update SQL */ true,
                            ...
                            {
                                Direction = SyncDirection.From,
                                NewPath = (syncSettings.CloudRoot ?? string.Empty) + "\\" + (currentEvent.Metadata.RelativePath ?? currentEvent.Metadata.RelativeToPath).Replace('/', '\\'),
                            ...
                            if (originalMetadata == null)
                            {
                                ...
                                if (syncStateMetadata == null)
                                {
                                    if (currentChange.Direction == SyncDirection.From)
                                    {
                                        ...
                                        {
                                            syncFromErrors.Add(new PossiblyStreamableAndPossiblyChangedFileChangeWithError(false,
                                                newPathCreation,
                                                null,
                                                new Exception("Unable to find metadata for file. May have been a rename on a local file path that does not exist. Created new FileChange to download file")));
        ...
        {
            string metadataRequestUri = CLDefinitions.CLMetaDataServerURL +
                (isFolder
                    ? CLDefinitions.MethodPathGetFolderMetadata
                    : CLDefinitions.MethodPathGetFileMetadata) +
            ...
            HttpWebRequest metadataRequest = (HttpWebRequest)HttpWebRequest.Create(metadataRequestUri);
            metadataRequest.Method = CLDefinitions.HeaderAppendMethodGet;
            metadataRequest.UserAgent = CLDefinitions.HeaderAppendCloudClient;
            // Add the client type and version.  For the Windows client, it will be Wnn.  e.g., W01 for the 0.1 client.
            metadataRequest.Headers[CLDefinitions.CLClientVersionHeaderName] = syncSettings.ClientVersion;
            metadataRequest.Headers[CLDefinitions.HeaderKeyAuthorization] = CLDefinitions.HeaderAppendToken + CLDefinitions.WrapInDoubleQuotes(syncSettings.Akey);
            metadataRequest.SendChunked = false;
            metadataRequest.Timeout = HttpTimeoutMilliseconds;
            ...
            if ((syncSettings.TraceType & TraceType.Communication) == TraceType.Communication)
            {
                Trace.LogCommunication(syncSettings.TraceLocation,
                    ...
                    null,
                    syncSettings.TraceExcludeAuthorization,
                    metadataRequest.Host,
                    null,
                    (metadataRequest.Expect == null ? "100-continue" : metadataRequest.Expect),
                    (metadataRequest.KeepAlive ? "Keep-Alive" : "Close"));
            }
            ...
            try
            {
                using (Stream metadataHttpWebResponseStream = metadataResponse.GetResponseStream())
                {
                    ...
                    Stream metadataResponseStream = (((storeTraceType & TraceType.Communication) == TraceType.Communication)
                        ? Helpers.CopyHttpWebResponseStreamAndClose(metadataHttpWebResponseStream)
                        : metadataHttpWebResponseStream);
        ...
        private static bool ContinueToRetry(PossiblyPreexistingFileChangeInError toRetry, ISyncDataObject syncData, byte MaxNumberOfFailureRetries, byte MaxNumberOfNotFounds)
        {
