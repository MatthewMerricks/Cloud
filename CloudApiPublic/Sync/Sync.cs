                    ...
                    for (int currentEventIndex = 0; currentEventIndex < deserializedResponse.Events.Length; currentEventIndex++)
                    {
                        ...
                        if (duplicatedEvents.BinarySearch(currentEventIndex) < 0)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    findNewPath = syncSettings.CloudRoot + "\\" +
                                        (currentEvent.Metadata.RelativePath ?? currentEvent.Metadata.RelativeToPath).Replace('/', '\\');
                                                        ...
                                                        CreateFileChangeFromBaseChangePlusHash(new FileChange()
                                                            {
                                                                ...
                                                                Metadata = new FileMetadata()
                                                                {
                                                                    ...
                                                                    LinkTargetPath = (newMetadata.TargetPath == null
                                                                        ? null
                                                                        : syncSettings.CloudRoot + "\\" + newMetadata.TargetPath.Replace("/", "\\"))
                                                                }
                                                            },
                                                            newMetadata.Hash),
        ...
        private JsonContracts.Metadata GetMetadataAtPath(FilePath getMetadataPath, bool isFolder)
        {
            string metadataRequestUri = CLDefinitions.CLMetaDataServerURL +
                ...
                Helpers.QueryStringBuilder(new[]
                                                        {
                                                            new KeyValuePair<string, string>(CLDefinitions.CLMetadataCloudPath, Uri.EscapeDataString(getMetadataPath.GetRelativePath(syncSettings.CloudRoot, true) + "/")),
