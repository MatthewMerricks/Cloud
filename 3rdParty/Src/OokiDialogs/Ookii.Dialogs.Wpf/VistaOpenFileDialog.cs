...
using Ookii.Dialogs.WpfMinusTaskDialog.Interop;
...
namespace Ookii.Dialogs.WpfMinusTaskDialog
{
    ...
    public sealed class VistaOpenFileDialog : VistaFileDialog
    {
        ...
        internal override Ookii.Dialogs.WpfMinusTaskDialog.Interop.IFileDialog CreateFileDialog()
        {
            return new Ookii.Dialogs.WpfMinusTaskDialog.Interop.NativeFileOpenDialog();
        }

        internal override void SetDialogProperties(Ookii.Dialogs.WpfMinusTaskDialog.Interop.IFileDialog dialog)
        {
            base.SetDialogProperties(dialog);
            if( _showReadOnly )
            {
                Ookii.Dialogs.WpfMinusTaskDialog.Interop.IFileDialogCustomize customize = (Ookii.Dialogs.WpfMinusTaskDialog.Interop.IFileDialogCustomize)dialog;
        ...
        internal override void GetResult(Ookii.Dialogs.WpfMinusTaskDialog.Interop.IFileDialog dialog)
        {
            if( Multiselect )
            {
                Ookii.Dialogs.WpfMinusTaskDialog.Interop.IShellItemArray results;
                ((Ookii.Dialogs.WpfMinusTaskDialog.Interop.IFileOpenDialog)dialog).GetResults(out results);
                uint count;
                results.GetCount(out count);
                string[] fileNames = new string[count];
                for( uint x = 0; x < count; ++x )
                {
                    Ookii.Dialogs.WpfMinusTaskDialog.Interop.IShellItem item;
