...
namespace ErikEJ.SqlCe
{
    ...
    public sealed partial class SqlCeBulkCopyFixture
    {
        ...
        //This test was commented out since the dataset only exists on one machine. please verify timing data.
        //[Test]
        public void ExerciseEngineWithDataReader()
        {
            CreateDatabase(SchemaType.DataReaderTest);
            using (SqlConnection conn = new SqlConnection("Data Source=EEJW7;Database=PfTest;Integrated Security=True"))
            {
                conn.Open();
