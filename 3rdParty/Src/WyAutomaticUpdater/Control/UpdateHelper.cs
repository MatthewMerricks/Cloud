...
namespace wyDay.Controls
{
    internal class UpdateHelper : IDisposable
    {
        ...
        {
            get { return CloudUpdaterLocation; }
            set
            {
        ...
        bool StartClient()
        {
            ...
            if (pipeClient.Connected)
            {
                ...
                if (!RetrySend((new UpdateHelperData(UpdateAction.GetwyUpdateProcessID)).GetByteArray()))
                    throw new Exception("Failed to get the CloudUpdater Process ID.");

                return true;
            }

            if (!File.Exists(m_CompleteWULoc))
                throw new Exception("The CloudUpdater executable was not found: " + m_CompleteWULoc);
        ...
        void bw_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                if (!StartClient())
                    throw new Exception("CloudUpdater failed to start.");
            }
