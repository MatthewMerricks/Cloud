...
namespace Microsoft.ApplicationServer.Serialization
{
    ...
    using Microsoft.ApplicationServer.Serialization.Configuration;
    using DataContractDictionary = System.Collections.Generic.Dictionary<System.Xml.XmlQualifiedName, DataContract>;

    internal abstract class DataContract
    {
        const string DataContractSerializerSectionPath = "system.runtime.serialization/dataContractSerializer";
        ...
        internal static DataContract GetGetOnlyCollectionDataContract(int id, RuntimeTypeHandle typeHandle, Type type, SerializationMode mode)
        {
            ...
            {
                throw Fx.Exception.AsError(new SerializationException(SR.ClassDataContractReturnedForGetOnlyCollection(DataContract.GetClrTypeFullName(dataContract.UnderlyingType))));
            }
            return dataContract;
        }
        ...
        protected class DataContractCriticalHelper
        {
            ...
            internal static DataContract GetDataContractForInitialization(int id)
            {
                ...
                if (dataContract == null)
                {
                    throw Fx.Exception.AsError(new SerializationException(SR.DataContractCacheOverflow));
                }
                return dataContract;
            }
                        ...
                        if (dataContract == null)
                        {
                            ...
                            else
                            {
                                ...
                                if (!CollectionDataContract.TryCreate(type, out dataContract))
                                {
                                    ...
                                    {
                                        ThrowInvalidDataContractException(SR.TypeNotSerializable(type), type);
                                    }
                                }
                            }
                        }
            ...
            internal static void ThrowInvalidDataContractException(string message, Type type)
            {
                ...
                throw Fx.Exception.AsError(new InvalidDataContractException(message));
            }
        ...
        static void ValidatePreviousCollectionTypes(Type collectionType, Type itemType, Dictionary<Type, object> previousCollectionTypes)
        {
            ...
            while (itemTypeQueue.Count > 0)
            {
                ...
                {
                    throw Fx.Exception.AsError(new InvalidDataContractException(SR.RecursiveCollectionType(DataContract.GetClrTypeFullName(itemType))));
                }
        ...
        internal static bool TryGetDCAttribute(Type type, out DataContractAttribute dataContractAttribute)
        {
            ...
            if (dataContractAttributes != null && dataContractAttributes.Length > 0)
            {
                ...
                if (dataContractAttributes.Length > 1)
                    throw Fx.Exception.AsError(new InvalidDataContractException(SR.TooManyDataContracts(DataContract.GetClrTypeFullName(type))));
#endif
        ...
        static XmlQualifiedName GetCollectionStableName(Type type, Type itemType, Dictionary<Type, object> previousCollectionTypes, out CollectionDataContractAttribute collectionContractAttribute)
        {
            ...
            if (collectionContractAttributes != null && collectionContractAttributes.Length > 0)
            {
                ...
                if (collectionContractAttributes.Length > 1)
                    throw Fx.Exception.AsError(new InvalidDataContractException(SR.TooManyCollectionContracts(DataContract.GetClrTypeFullName(type))));
#endif
                collectionContractAttribute = (CollectionDataContractAttribute)collectionContractAttributes[0];
                // ALTERED_FOR_PORT: Don't have access to .IsSetExplicilty property
                if (collectionContractAttribute.Name != null && collectionContractAttribute.Name.Length > 0)
                {
                ...
                if (collectionContractAttribute.Namespace != null)
                {
        ...
        static void CheckExplicitDataContractNamespaceUri(string dataContractNs, Type type)
        {
            if (dataContractNs.Length > 0)
            {
                ...
                if (trimmedNs.Length == 0 || trimmedNs.IndexOf("##", StringComparison.Ordinal) != -1)
                    ThrowInvalidDataContractException(SR.DataContractNamespaceIsNotValid(dataContractNs), type);
            ...
            if (Uri.TryCreate(dataContractNs, UriKind.RelativeOrAbsolute, out uri))
            {
                if (uri.ToString() == Globals.SerializationNamespace)
                    ThrowInvalidDataContractException(SR.DataContractNamespaceReserved(Globals.SerializationNamespace), type);
            }
            else
                ThrowInvalidDataContractException(SR.DataContractNamespaceIsNotValid(dataContractNs), type);
        }
        ...
        static string GetClrTypeFullNameForNonArrayTypes(Type type)
        {
            ...
            foreach (Type genericArgument in genericArguments)
            {
                ...
                builder.Append(Globals.Space).Append(GetClrAssemblyName(genericArgument));
            ...
            for (int i = 0; i < nsAttributes.Length; i++)
            {
                ...
                if (clrNsInAttribute == clrNs)
                {
                    if (nsAttribute.ContractNamespace == null)
                        throw Fx.Exception.AsError(new InvalidDataContractException(SR.InvalidGlobalDataContractNamespace(clrNs)));
                    if (dataContractNs != null)
                        throw Fx.Exception.AsError(new InvalidDataContractException(SR.DataContractNamespaceAlreadySet(dataContractNs, nsAttribute.ContractNamespace, clrNs)));
                ...
                if (ch == '{')
                {
                    ...
                    for (; i < format.Length; i++)
                        if (format[i] == '}')
                            break;
                    if (i == format.Length)
                        throw Fx.Exception.AsError(new InvalidDataContractException(SR.GenericNameBraceMismatch(format, genericNameProvider.GetGenericTypeName())));
                    ...
                    else
                    {
                        ...
                        if (!Int32.TryParse(format.Substring(start, i - start), out paramIndex) || paramIndex < 0 || paramIndex >= genericNameProvider.GetParameterCount())
                            throw Fx.Exception.AsError(new InvalidDataContractException(SR.GenericParameterNotValid(format.Substring(start, i - start), genericNameProvider.GetGenericTypeName(), genericNameProvider.GetParameterCount() - 1)));
        ...
        static DataContractSerializerSection ConfigSection
        {
            [Fx.Tag.SecurityNote(Critical = "Elevates in order to get the DataContractSerializerSection config section and stores result in"
            ...
            get
            {
                ...
                {
                    configSection = (DataContractSerializerSection)ConfigurationManager.GetSection(DataContractSerializerSectionPath);
                    if (configSection == null)
                    {
                        throw Fx.Exception.AsError(new ConfigurationErrorsException(SR.ConfigDataContractSerializerSectionLoadError));
                    }
                }
                ...
                if (knownTypeAttributes != null)
                {
                    ...
                    for (int i = 0; i < knownTypeAttributes.Length; ++i)
                    {
                        ...
                        if (kt.Type != null)
                        {
                            if (useMethod)
                            {
                                DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeOneScheme(DataContract.GetClrTypeFullName(type)), type);
                            }
                        ...
                        else
                        {
                            ...
                            if (methodName == null)
                            {
                                DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeNoData(DataContract.GetClrTypeFullName(type)), type);
                            }

                            if (methodName.Length == 0)
                                DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeEmptyString(DataContract.GetClrTypeFullName(type)), type);
                            ...
                            if (method == null)
                                DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeUnknownMethod(methodName, DataContract.GetClrTypeFullName(type)), type);

                            if (!Globals.TypeOfTypeEnumerable.IsAssignableFrom(method.ReturnType))
                                DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeReturnType(DataContract.GetClrTypeFullName(type), methodName), type);

                            object types = method.Invoke(null, Globals.EmptyObjectArray);
                            if (types == null)
                            {
                                DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeMethodNull(DataContract.GetClrTypeFullName(type)), type);
                            }

                            foreach (Type ty in (IEnumerable<Type>)types)
                            {
                                if (ty == null)
                                    DataContract.ThrowInvalidDataContractException(SR.KnownTypeAttributeValidMethodTypes(DataContract.GetClrTypeFullName(type)), type);
        ...
        private static void CheckRootTypeInConfigIsGeneric(Type type, ref Type rootType, ref Type[] genArgs)
        {
            if (rootType.IsGenericType)
            {
                ...
                {
                    DataContract.ThrowInvalidDataContractException(SR.TypeMustBeConcrete(type), type);
                }
            }
        }
        ...
        private static bool IsCollectionElementTypeEqualToRootType(string collectionElementTypeName, Type rootType)
        {
            if (collectionElementTypeName.StartsWith(DataContract.GetClrTypeFullName(rootType), StringComparison.Ordinal))
            {
                ...
                if (t != null)
                {
                    ...
                    {
                        throw Fx.Exception.AsError(new ArgumentException(SR.KnownTypeConfigClosedGenericDeclared(collectionElementTypeName)));
                    }
        ...
        internal static void CheckAndAdd(Type type, Dictionary<Type, Type> typesChecked, ref DataContractDictionary nameToDataContractTable)
        {
            ...
            else if (nameToDataContractTable.TryGetValue(dataContract.StableName, out alreadyExistingContract))
            {
                if (alreadyExistingContract.UnderlyingType != DataContractCriticalHelper.GetDataContractAdapterType(type))
                    throw Fx.Exception.AsError(new InvalidOperationException(SR.DupContractInKnownTypes(type, alreadyExistingContract.UnderlyingType, dataContract.StableName.Namespace, dataContract.StableName.Name)));
                return;
            }
