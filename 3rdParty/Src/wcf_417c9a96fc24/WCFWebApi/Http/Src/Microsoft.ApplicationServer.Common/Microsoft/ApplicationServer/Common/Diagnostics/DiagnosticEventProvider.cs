...
namespace Microsoft.ApplicationServer.Common.Diagnostics
{
    ...
    class DiagnosticsEventProvider : IDisposable
    {
        ...
        {
            // TODO, 91862, Investigate if this is the right way to fetch ActivityId.
            // Should we use CorrelationManager or should we use ETW API's as in case of SetActivityId
            object id = System.Diagnostics.Trace.CorrelationManager.ActivityId;
            return id == null ? Guid.Empty : (Guid)id;
        }

        [SuppressMessage(FxCop.Category.Usage, FxCop.Rule.DoNotIgnoreMethodResults, Scope = "Member", Target = "Microsoft.ApplicationServer.Common.Diagnostics.DiagnosticsEventProvider.#SetActivityId(System.Guid&)")]        
        [SecurityCritical]
        ...
        [SuppressMessage(FxCop.Category.Usage, FxCop.Rule.DoNotIgnoreMethodResults, Scope = "Member", Target = "Microsoft.ApplicationServer.Common.Diagnostics.DiagnosticsEventProvider.#CreateActivityId()")]
        [SecurityCritical]
