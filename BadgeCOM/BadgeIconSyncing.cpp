...
void CBadgeIconSyncing::OnEventSubscriptionWatcherFailed()
{
	try
	{
		...
		_delayedMethodTimer.SetTimedEvent(this, &CBadgeIconSyncing::OnDelayedEvent);
...
void CBadgeIconSyncing::OnDelayedEvent()
{
	try
	{
		...
		if (_pBadgeNetPubSubEvents != NULL)
		{
    		...
    		CLTRACE(9, "CBadgeIconSyncing: OnDelayedEvent: Restart the subscription.");
	...
	{
		CLTRACE(1, "CBadgeIconSyncing: OnDelayedEvent: ERROR: Exception.  Message: %s.", ex.what());
	}
}
