...
void CBadgeIconSelective::OnEventSubscriptionWatcherFailed()
{
	try
	{
		...
		_delayedMethodTimer.SetTimedEvent(this, &CBadgeIconSelective::OnDelayedEvent);
...
void CBadgeIconSelective::OnDelayedEvent()
{
	try
	{
		...
		if (_pBadgeNetPubSubEvents != NULL)
		{
    		...
    		CLTRACE(9, "CBadgeIconSelective: OnDelayedEvent: Restart the subscription.");
	...
	{
		CLTRACE(1, "CBadgeIconSelective: OnDelayedEvent: ERROR: Exception.  Message: %s.", ex.what());
	}
}
